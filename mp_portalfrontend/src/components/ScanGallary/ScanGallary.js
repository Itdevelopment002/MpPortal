...
const ScanGallery = () => {
    ...
    const handleDownloadPDF = async (entryId) => {
        ...
        if (entry) {
            ...
            doc.rect(tableX + columnWidths[0], startY, columnWidths[1], rowHeight, 'FD'); 

            // Header Text
            doc.setFontSize(12);
            doc.setTextColor(255, 255, 255);
            doc.text('Field Name', tableX + 5, startY + 7);
            doc.text('Value', tableX + columnWidths[0] + columnWidths[1] / 2, startY + 7, null, null, 'center');
            ...
            Object.keys(entry).forEach((key) => {
                ...
                doc.rect(tableX + columnWidths[0], yPosition, columnWidths[1], rowHeight, 'FD'); 

                doc.setFontSize(11);
                doc.setTextColor(0, 0, 0); 
                doc.text(key, tableX + 5, yPosition + 7); 
                doc.text(`${value}`, tableX + columnWidths[0] + columnWidths[1] / 2, yPosition + 7, null, null, 'center'); 
            ...
            doc.setDrawColor(0, 0, 0); 
            doc.rect(tableX, startY, columnWidths[0] + columnWidths[1], totalHeight + rowHeight);

            doc.setFontSize(10);
            doc.setTextColor(128, 128, 128); 
            doc.text('Generated using our system.', 105, yPosition + 10, null, null, 'center'); 
    ...
    const handleCopy = async (entryId) => {
        ...
        if (entry) {
            ...
            navigator.clipboard.writeText(textToCopy)
                .then(() => {
                    showCopyMessage('🎉 Data for Entry ' + entryId + ' has been copied successfully!');
                })
                .catch((err) => {
                    console.error('Error copying text: ', err);
                    showCopyMessage('❌ Failed to copy data to clipboard. Please try again!');
                });
        }
    };
    ...
    const handlePrint = async (entryId) => {
        ...
        if (entry) {
            ...
            const formattedEntry = Object.keys(entry).reduce((acc, key) => {
                acc[key] = key.toLowerCase().includes('date') && entry[key]
                    ...
                    <div>
                        <nav>
                            <ol className="breadcrumb mb-1">
                                ...
                                <li className="breadcrumb-item active" aria-current="page">Entries</li>
                            </ol>
                        </nav>
                    </div>
                </div>
                        ...
                        <div key={entry.id} className="col-lg-4 col-md-6 col-sm-12 mb-5">
                            <div className="custom-card">
                                ...
                                <div className="custom-card-body">
                                    {Object.keys(entry)
                                        .filter((key) => key !== "user_id" && key !== "created_at" && key !== "id")
                                        .map((key) => (
                                            <p key={key}>
                                                <strong>
                                                    {key.charAt(0).toUpperCase() + key.slice(1)}:
                                                </strong>
                                                {key === "entry_date" ? formatDate(entry[key]) : entry[key]}
                                            </p>
                                        ))}
                                </div>
